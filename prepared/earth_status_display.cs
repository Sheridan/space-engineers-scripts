...
result+=$"Consuming(now,max):{tHR(powerConsumed,EHRUnit.Power)}:{tHR(powerMaxConsumed,EHRUnit.Power)}";}
return result;}
...
result+=$"Capacity:{tHR(capacity,EHRUnit.Volume)}"
...
maxStored*=1000000;
result+=$"Capacity:{tHR(currentStored,EHRUnit.PowerCapacity)}:{tHR(maxStored,EHRUnit.PowerCapacity)}";
return result;}
...
if(inventoryes > 0){
mass*=1000;
return $"VMI:({tHR(volume,EHRUnit.Volume)}:{tHR(volumeMax,EHRUnit.Volume)}):{tHR(mass,EHRUnit.Mass)}:{tHR(items)}from{inventoryes}";}
return "";}
...
result+=$"Ген.энергии(now:max):{tHR(currentOutput,EHRUnit.Power)}:{tHR(maxOutput,EHRUnit.Power)}";
return result;}
...
return "";}
public static string tHR(float value,EHRUnit unit=EHRUnit.None){
...
public CBT<IMyPowerProducer> h2Engines;
...
windTurbines=new CBT<IMyPowerProducer>("WindTurbine");
solarPanels=new CB<IMySolarPanel>();
