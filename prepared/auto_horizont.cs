...
class CShipController
{
  ...
  private double calcAngle(Vector3D planeNormal)
  {
    Vector3D gravity = Vector3D.Normalize(m_controller.GetNaturalGravity());
    Vector3D position = Vector3D.Normalize(m_controller.GetPosition());
    Vector3D projected = Vector3D.ProjectOnPlane(ref gravity, ref planeNormal);
    return angleBetweenVectors(position, projected);
  }
  public double pitchAngle()
  {
    return calcAngle(Vector3D.Forward);
  }
  public double rollAngle()
  {
    return calcAngle(Vector3D.Up);
  }
...
public static double angleBetweenVectors(Vector3D v1, Vector3D v2)
{
  ...
  return MathHelper.ToDegrees(Math.Acos(v1Norm.Dot(v2Norm) / (v1Norm.Length() * v2Norm.Length())));
}
...
{
  lcd.showStatus<IMyGyro>(gyroscopes, 0);
  lcd.echo_at($"{controller.pitchAngle():f8}", 1);
  lcd.echo_at($"{controller.rollAngle():f8}", 2);
}
